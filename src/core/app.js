...
import { createPost, getPosts, voteOnPost, addComment, getComments, deletePost, editPost } from '../features/voting/posts.js';
import { createProject, voteOnProject } from '../features/projects/projects.js';
import { syncLegislation, inspectBill, listAllBills, resetSyncCooldown } from '../services/Legislation.js';
import { importFirstBill, importAllBills } from '../tools/data-import/importBills.js';
...
// ═══════════════════════════════════════════════════════════════════════════
// PAGINATION: Variables for batch loading - MUST be at module scope
// ═══════════════════════════════════════════════════════════════════════════
let allFeedItems = [];
let currentPostIndex = 0;
const INITIAL_BATCH_SIZE = 40;
const SCROLL_BATCH_SIZE = 20;
...
function getFollowButtonHtml(authorId, authorName) {
    ...
    return `<button class="follow-btn" onclick="window.handleFeedFollow('${authorId}', this)">Follow</button>`;
}

// Cache for author data to avoid repeated fetches
const authorCache = {};

// Helper function to get user's vote direction for a post
async function getUserVoteStatus(postId) {
    try {
        if (!window.currentUserId) return null;
        const voteRef = doc(db, 'feed', postId, 'votes', window.currentUserId);
        const voteDoc = await getDoc(voteRef);
        if (voteDoc.exists() && voteDoc.data().type) {
            return voteDoc.data().type; // 'up' or 'down'
        }
        return null;
    } catch (err) {
...
// Apply vote highlighting and comment counts after posts render
async function applyPostEnhancements(postIds) {
    try {
        for (const postId of postIds) {
            const voteStatus = await getUserVoteStatus(postId);
            const commentCount = await getCommentCount(postId);
            
            // Find vote buttons by looking through all buttons in post container
            const buttons = document.querySelectorAll('button[onclick*="handleVote"]');
            for (let btn of buttons) {
                const onclick = btn.getAttribute('onclick');
                if (onclick && onclick.includes(`'${postId}'`)) {
                    if (onclick.includes(`'up'`) && voteStatus === 'up') {
                        btn.style.color = '#000';
                        btn.style.fontWeight = '700';
                        btn.style.background = 'var(--color-progressBar)';
                    } else if (onclick.includes(`'down'`) && voteStatus === 'down') {
                        btn.style.color = '#000';
                        btn.style.fontWeight = '700';
                        btn.style.background = 'var(--color-delete)';
                    }
                }
            }
            
            // Apply comment count badge
            if (commentCount > 0) {
                const commentBtns = document.querySelectorAll(`button[onclick*="toggleComments('${postId}')"]`);
                for (let btn of commentBtns) {
                    // Check if badge already exists
                    if (btn.querySelector('span[style*="position:absolute"]')) continue;
                    
                    const badge = document.createElement('span');
                    badge.textContent = commentCount;
                    badge.style.cssText = 'position:absolute; top:-8px; right:-8px; background:#dc3545; color:white; border-radius:50%; width:20px; height:20px; display:flex; align-items:center; justify-content:center; font-size:16px; font-weight:700; padding:8px;';
        ...
        console.error('Error applying post enhancements:', err);
    }
}

// Scroll handler to load more posts
function handleScrollPagination() {
    if (isLoadingMore || currentPostIndex >= allFeedItems.length) return;
    
    const scrollPercentage = (window.scrollY + window.innerHeight) / document.documentElement.scrollHeight;
    
    // Load more when user is 75% through the page
    if (scrollPercentage > 0.75) {
        loadMorePosts();
    }
}

// Load and render next batch of posts
async function loadMorePosts() {
    ...
    const startIndex = currentPostIndex;
    const endIndex = Math.min(currentPostIndex + SCROLL_BATCH_SIZE, allFeedItems.length);
    ...
    await reloadPostsWithPagination();
    
    currentPostIndex = endIndex;
    isLoadingMore = false;
    
    // Remove scroll listener if all posts loaded
    if (currentPostIndex >= allFeedItems.length) {
        window.removeEventListener('scroll', handleScrollPagination);
    }
}

// Re-render the feed with pagination
async function reloadPostsWithPagination() {
    try {
        const container = document.getElementById('posts');
        if (!container) return;
        
        // Determine how many posts to show
        const postsToShow = Math.min(currentPostIndex + SCROLL_BATCH_SIZE, allFeedItems.length);
        const visibleItems = allFeedItems.slice(0, postsToShow);
        
        // Just re-render from scratch (simpler than appending)
        container.innerHTML = visibleItems.map(function(item) {
            ...
            if (item.type === 'legislation') {
                ...
                return `
                    <div class="post">
                        <div class="post-header">
                            ${avatarHtml}
                            <div class="post-meta">
                                <div class="author"><a href="profile.html?uid=${item.authorId || ''}">${author}</a></div>
                                <div class="post-meta-right">
                                    ${getFollowButtonHtml(item.authorId, displayName)}
                        ...
                        <div class="project-tag-container" style="background:${tabColor};">
                            <a href="src/features/projects/projectpage/project.html?id=${item.id}&type=legislation" style="color:#fff; text-decoration:none;">
                                <div class="project-tag-badge" style="background:${tabColor};">${item.category || 'Bill'}</div>
                ...
                const headerImage = item.headerPictureUrl ? `<img src="${item.headerPictureUrl}" style="min-width:100%; max-width:400px; height:auto; max-height:800px; border-radius:6px; margin:6px 0; display:block;">` : '';
                ...
                if (media.image) mediaHtml += `<img src="${media.image}" style="min-width:100%; max-width:400px; border-radius:4px; margin:8px 0; max-height:800px;">`;
                ...
                if (media.video) mediaHtml += `<video width="100%" min-width="100%" controls style="border-radius:4px; margin:8px 0; max-height:800px;"><source src="${media.video}"></video>`;
                if (media.link) {
                    const isValidUrl = /^https?:\/\//.test(media.link);
                    const displayUrl = media.link.replace(/^https?:\/\//, '').substring(0, 60);
                    mediaHtml += isValidUrl
                ...
                return `
                    <div class="post">
                        ...
                        <div style="padding:8px; background:#e0e7ff; border-radius:4px; margin-bottom:8px; font-size:21px; color:#1e40af; font-weight:600;">
                        ...
                        <h3 style="margin:0 0 8px; font-size:21px;">${item.title}</h3>
                        <p style="margin:0 0 8px; font-size:21px; line-height:1.5;">${item.content}</p>
                        ${mediaHtml}
                        <div class="post-actions">
                            <div class="votes">
                                <button onclick="handleVote('${item.id}', 'up', 'post')">▲</button>
                ...
                return `
                    <div class="post">
                        ...
                        <div style="padding:8px; background:#f3e8ff; border-radius:4px; margin:6px 0; font-size:21px; color:#6b21a8; font-weight:600;">
                ...
                return `
                    <div class="post">
                        ...
                        <div style="padding:8px; background:#9ca3af; border-radius:4px; margin:6px 0; font-size:21px; color:#fff; font-weight:600;">
                ...
                return `<div class="post"><p>Post type "${item.type}" not recognized</p></div>`;
            }
        }).join('');
        
        // Apply enhancements to visible posts
        const postIds = visibleItems.map(item => item.id);
        if (postIds.length > 0) {
            applyPostEnhancements(postIds);
        }
    } catch (error) {
        console.error('Error reloading posts with pagination:', error);
    }
}
...
async function loadPosts() {
    try {
        const { getFeed } = await import('../features/projects/projects.js');
        ...
        // Render only the first batch (40 posts)
        const initialItems = items.slice(0, INITIAL_BATCH_SIZE);
        currentPostIndex = initialItems.length;
        
        container.innerHTML = initialItems.map(function(item) {
        const createdAtDate = item.createdAt ? new Date(item.createdAt.seconds * 1000) : null;
        const date = createdAtDate ? timeAgo(createdAtDate) : 'Just now';
    ...
    // Collect post IDs and apply enhancements
    const postIds = initialItems.map(item => item.id);
    if (postIds.length > 0) {
        applyPostEnhancements(postIds);
    }
    
    // Set up scroll listener if there are more posts
    if (allFeedItems.length > INITIAL_BATCH_SIZE) {
...
// Expose post enhancement function for manual triggering
window.applyPostEnhancements = applyPostEnhancements;
window.getUserVoteStatus = getUserVoteStatus;
