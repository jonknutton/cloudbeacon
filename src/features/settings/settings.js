...
const AccountSettings = {
    ...
    async openSettingsModal() {
        ...
        // Clear stale account details from previous user
    ...
    switchSettingsTab(tabName) {
        ...
        if (tabName === 'account') {
            // Clear old data first, then load current user's data
            this.clearAccountDetails();
    ...
    async loadAccountDetails() {
        try {
            const user = window.auth.currentUser;
            if (!user) {
                // User is not authenticated - clear all fields
                this.clearAccountDetails();
                return;
            }
            ...
            // Remove any stale lock notes before checking for lock
            const staleNote = document.getElementById('countryLockNote');
            if (staleNote) staleNote.remove();
            
            // Unlock country field first, then lock if needed
            ...
            // Lock country field if it's already been set
            if (userData.country) {
                this.lockCountryField();
            }
        } catch (error) {
    ...
    /**
     * Clear account details form
     */
    clearAccountDetails() {
        const fields = ['accountUserID', 'accountEmail', 'accountFullName', 'accountCountry', 'accountAddress', 'accountPhone'];
        fields.forEach(id => {
            const elem = document.getElementById(id);
            if (elem) elem.value = '';
        });
        
        // Remove lock note if it exists
        const lockNote = document.getElementById('countryLockNote');
        if (lockNote) lockNote.remove();
        
        // Unlock country field
        const countrySelect = document.getElementById('accountCountry');
        if (countrySelect) {
            countrySelect.disabled = false;
            countrySelect.style.opacity = '1';
    ...
    async saveAccountDetails() {
        try {
            ...
            // Check if this is the first time setting country
            const existingDoc = await getDoc(doc(db, 'users', user.uid));
            const existingData = existingDoc.data() || {};
            const isFirstCountrySave = !existingData.country && country;
            
            // Show warning if setting country for first time
            if (isFirstCountrySave) {
                const confirmSet = confirm(
                    `⚠️ Important: You're setting your country to ${country}.\n\n` +
                    `After saving, you can only change this by requesting admin approval.\n\n` +
                    `Your IP address will be verified to match this country for security.\n\n` +
                    `Are you sure you want to continue?`
                );
                
                if (!confirmSet) {
                    console.log('[Settings] Country save cancelled by user');
                    return;
                }
            }
            ...
            // If first country save, add timestamp for reference
            if (isFirstCountrySave) {
                updateData.countrySetAt = new Date().toISOString();
            }
            ...
            // Lock country field if it was just set
            if (isFirstCountrySave) {
                this.lockCountryField();
            }
    ...
    /**
     * Lock country field - only admin can change it
     */
    lockCountryField() {
        const countrySelect = document.getElementById('accountCountry');
        ...
        // Add a note below the country field
        let note = document.getElementById('countryLockNote');
        if (!note) {
            note = document.createElement('div');
            note.id = 'countryLockNote';
            note.style.cssText = `
                font-size: 12px;
                color: #ff9800;
                margin-top: 6px;
                font-weight: 500;
            `;
            countrySelect.parentNode.insertBefore(note, countrySelect.nextSibling);
        }
        
        note.innerHTML = `
            🔒 Country is locked for security. 
            <a href="#" onclick="AccountSettings.requestCountryChange(event)" style="color: #ff9800; text-decoration: underline;">Request change</a>
        `;
        
        console.log('[Settings] Country field locked');
    },

    /**
     * Request country change - opens support channel
     */
    requestCountryChange(event) {
        event.preventDefault();
        const currentCountry = document.getElementById('accountCountry').value;
        alert(
            `To change your country from ${currentCountry}:\n\n` +
            `1. Contact support with your request\n` +
            `2. Provide your new country\n` +
            `3. An admin will review and approve\n\n` +
            `Note: IP geolocation must match your new country for approval.`
        );
    }
};
